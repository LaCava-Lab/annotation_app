...
if "subpages" not in st.session_state:
    ...
    st.session_state.labels_subpages = [{"label": item["label"]} for item in st.session_state.subpages]
    st.session_state.current_page = {"subpage": st.session_state.labels_subpages[0], "index": 0}
...
for i,subpage in enumerate(st.session_state.subpages):
    ...
    subpages_data.append(Subpage(label, doi_link, paper_data, sidebar_content, selections, highlighter_labels, coffee_break))
...
def changePage(index):
    st.session_state.current_page = {
        "subpage": st.session_state.labels_subpages[index],
        "index": index
    }
...
def next():
    if st.session_state.current_page["index"] < len(st.session_state.labels_subpages) - 1:
...
def save():
    ...
    #SAVE EXPERIMENTS
    if index < len(st.session_state.subpages):
...
pageSelected = BreadCrumbs(st.session_state.labels_subpages, st.session_state.current_page["subpage"], pages=st.session_state.subpages)


# label = st.session_state.subpages[0]["label"]
# sidebar_content = st.session_state.subpages[0]["sidebar_content"]
# coffee_break = st.session_state.subpages[0]["coffee_break"]
# highlighter_labels = st.session_state.subpages[0]["highlighter_labels"]
# test_page = Subpage(label,doi_link,paper_data,sidebar_content,highlighter_labels,coffee_break)
# print(test_page.label, test_page.coffee_break)

# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# UI CODE
# MAIN PAGE
# st.write(test_page.selections)
# print(st.session_state.current_subpage)
# pageSelected = BreadCrumbs(st.session_state.subpages, st.session_state.current_subpage["page"], pages=st.session_state.subpages)
# test_page.main_page(tab_names)
