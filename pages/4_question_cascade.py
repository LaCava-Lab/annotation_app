...
# Load the selected paper's metadata from Papers.csv (CSV -> JSON -> parsing)
paper_meta = get_paper_metadata_by_pmid(pmid)
if not paper_meta:
    st.error("Metadata for this paper not found.")
    st.stop()
...
if issue:
    metadata_parts.append(f"**Issue:** {issue}")
if volume:
    metadata_parts.append(f"**Volume:** {volume}")
if pages:
    metadata_parts.append(f"**Pages:** {pages}")
if year:
